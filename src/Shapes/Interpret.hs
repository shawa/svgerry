...
toAttr (StrokeWidth  w) = strokeWidth $ stringValue $show w
toAttr (StrokeColour c) = stroke      $ stringValue $ show c
toAttr (FillColour   c) = fill        $ stringValue $ show c
...
toAttrs t = transform $ matrix a b c d e f where [ [a, c, e] ,[b, d, f] ,[0, 0, 1]] = M.toLists $ getMatrix t
...
svgHead = docTypeSvg ! version "1.1" ! width "1200" ! height "550" ! viewbox "0 0 3 2"
